// Pure-JS MD5 for use in non-secure contexts where crypto.subtle is unavailable.
// Based on the algorithm by Paul Johnston (pajhome.org.uk/crypt/md5).
function md5hex(str: string): string {
  ...
  for (let i = 0; i < blks.length; i += 16) cycle(state, blks.slice(i, i + 16));
...
export function libravatarUrl(email: string, size = 80, fallback = 'identicon'): string {
